...
async function init() {
  ...
  // ===== 强制确保模块定义存在（修复权限为空的情况）=====
  const modCount = await db.prepare('SELECT COUNT(*) as c FROM modules').get();
  if (!modCount || modCount.c === 0) {
    console.log('📝 插入模块定义...');
    const mList = [
      ['dashboard','数据看板','业务',1],['quotation-engine','报价引擎','业务',2],
      ['quote-doc','报价单生成','业务',3],['contract','合同生成','业务',4],
      ['schedule','排产计算','生产',5],['production-tracking','生产跟踪','生产',6],
      ['customer','客户档案','业务',7],['material','基材管理','基础数据',8],
      ['formula','配方管理','基础数据',9],['shape','模切形状','基础数据',10],
      ['formula-calc','配方计算','基础数据',11],['supplier','供方信息','基础数据',12],
      ['user-manager','用户管理','系统',13],['audit-log','操作日志','系统',14],
    ];
