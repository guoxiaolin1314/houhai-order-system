...
router.post('/:collection', async (req, res) => {
  try {
    ...
    const dbType = process.env.DATABASE_URL ? 'pg' : 'sqlite';
    if (dbType === 'pg') {
      await db.prepare(`INSERT INTO biz_${collection} (id, data, created_by) VALUES (?, ?::jsonb, ?)
        ON CONFLICT(id) DO UPDATE SET data = excluded.data, updated_at = NOW()`).run(itemId, data, req.user.uid);
    } else {
      await db.prepare(`INSERT INTO biz_${collection} (id, data, created_by) VALUES (?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET data = excluded.data, updated_at = datetime('now','localtime')`).run(itemId, data, req.user.uid);
    }
