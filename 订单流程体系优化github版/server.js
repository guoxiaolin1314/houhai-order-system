...
const fs = require('fs');
// 动态检测文件目录：兼容 Zeabur 子目录部署
let STATIC_DIR = __dirname;
const SUBDIR = '订单流程体系优化github版';
if (!fs.existsSync(path.join(STATIC_DIR, 'login.html')) && fs.existsSync(path.join(STATIC_DIR, SUBDIR, 'login.html'))) {
  STATIC_DIR = path.join(STATIC_DIR, SUBDIR);
}
